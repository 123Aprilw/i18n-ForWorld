<template>
	<view class="agreement-page">
		<PageHeader show-lang hide-title hide-border back-color="#0058BE" background="#F8F9FF" />
		<scroll-view scroll-y class="agreement-body" :show-scrollbar="false">
			<view class="agreement-main">
				...
				<!-- 如果API返回了内容，渲染API内容；否则使用默认的国际化文本 -->
				<view v-if="agreementContent" class="agreement-card">
				...
				<view v-else class="agreement-card">
					<view class="agreement-section">
						<view class="agreement-section__head">
							<image class="agreement-section__icon agreement-section__icon--terms"
								:src="icons.agreement.terms" mode="aspectFit" />
							<text
								class="agreement-section__title agreement-section__title--single">{{ t('agreement.section1Title') }}</text>
						</view>
					...
					<view class="agreement-section">
						<view class="agreement-section__head">
							<image class="agreement-section__icon agreement-section__icon--user"
								:src="icons.agreement.user" mode="aspectFit" />
							<text
								class="agreement-section__title agreement-section__title--single">{{ t('agreement.section2Title') }}</text>
						</view>
					...
					<view class="agreement-section agreement-section--privacy">
						<view class="agreement-section__head agreement-section__head--tall">
							<image class="agreement-section__icon agreement-section__icon--privacy"
								:src="icons.agreement.privacy" mode="aspectFit" />
					...
					<view class="agreement-section">
						<view class="agreement-section__head">
							<image class="agreement-section__icon agreement-section__icon--warning"
								:src="icons.agreement.warning" mode="aspectFit" />
							<text
								class="agreement-section__title agreement-section__title--single">{{ t('agreement.section4Title') }}</text>
						</view>
...
<script setup lang="ts">
	...
	onLoad((options) => {
		...
		// 获取协议详情
		fetchAgreementDetail(type)
	})

	const fetchAgreementDetail = async (type : string) => {
		loading.value = true
		try {
			const response = await api.GetAgreementDetail(type) as any
