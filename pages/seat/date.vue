...
<script setup lang="ts">
	...
	const { checkUnpaidOrder } = useReservationOrder()
	...
	const hasUnpaidOrder = ref(false)
	...
	const hasUnpaid = computed(() => hasUnpaidOrder.value || orders.some(o => o.status === 'pending'))
	...
	onShow(async () => {
		try {
			const unpaidOrder = await checkUnpaidOrder()
			hasUnpaidOrder.value = !!unpaidOrder
		} catch (error) {
	...
	const days = computed(() => {
		...
		for (let d = 1; d <= totalDays; d++) {
			...
			// 判断是否是过去日期
			if (currentYear < todayY) {
				isPast = true
			} else if (currentYear === todayY && currentMonth < todayM) {
				isPast = true
			} else if (currentYear === todayY && currentMonth === todayM && d < todayD) {
				isPast = true
			}
